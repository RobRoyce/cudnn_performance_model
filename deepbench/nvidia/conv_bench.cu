...
public:
    ...
    cudnnCNN(int w, int h, int c, int n, int k, int r, int s,
        ...
        if (!inference) {
            ...
            bwd_params_workspace_ = zeros<float>(
                    std::vector < int > {static_cast<int>(bwd_params_workspace_size_ / sizeof(float)), 1});
            ...
            bwd_inputs_workspace_ = zeros<float>(
                    std::vector < int > {static_cast<int>(bwd_inputs_workspace_size_ / sizeof(float)), 1});
        }
...
std::tuple<int, int, int, std::string> time_cnn(
    ...
    if (!inference) {
        ...
        bwd_params_time = static_cast<int>(std::chrono::duration<double, std::micro>(end - start).count() /
                                           num_repeats);
        ...
        bwd_inputs_time = static_cast<int>(std::chrono::duration<double, std::micro>(end - start).count() /
                                           num_repeats);
    }
...
int main(int argc, char **argv) {
    std::cout << "Usage: ./conv_bench [inference | train] [float | int8 | half] parameter_file" << std::endl;
    std::string params;
    int num_repeats = 300;
    ...
    if (argc < 4) {
        std::cout << "Must include parameter file as third argument!" << std::endl;
        return 1;
    } else {
        params = std::string(argv[3]);
    }
    ...
    std::cout
            << "   w      h      c      n      k      f_w    f_h  pad_w  pad_h    stride_w  stride_h    precision  fwd_time (usec)  ";
    ...
    /// Insert CSV file reader here to populate inference_server_set and training_set
    std::cout << "Reading convolution parameters from file " << params << std::endl;
    std::fstream fin(params, std::ios::in);
    ...
    while (!fin.eof()) {
        ...
        while (std::getline(s, word, ',')) {
            val = std::strtoul(word.c_str(), nullptr, 10);
            row.push_back(val);
        }


        if (inference) {
            inference_server_set.push_back(std::make_tuple(
                    row[0],
                    row[1],
                    row[2],
                    row[3],
            ...
            training_set.push_back(std::make_tuple(
                    row[0],
                    row[1],
                    row[2],
                    row[3],
    ...
    printf("Captured %d convolution configurations...\n", inference_server_set.size());
    /// Insert CSV file reader here to populate inference_server_set and training_set
    ...
    for (const auto &problem : (inference ? inference_server_set : training_set)) {
        ...
        if (precision != "float")
            throw std::runtime_error(ss.str());
        std::tie(fwd_time, bwd_inputs_time, bwd_params_time, fwd_algo_s) =
                time_cnn<float, float>(k, c, r, s, n, h, w, pad_h, pad_w, hstride, wstride, num_repeats, curand_gen,
                                       inference);
#endif
