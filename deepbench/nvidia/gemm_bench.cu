...
int main(int argc, char **argv) {
    ...
    std::cout << "Usage: gemm_bench [inference | train] [float | int8 | half] parameter_file" << std::endl;
    std::string params;
    ...
    std::cout << "Reading gemm parameters from file " << params << std::endl;
    ...
    while (!fin.eof()) {
        ...
        if (inference) {
            inference_server_set.push_back(std::make_tuple(row[0], row[1], row[2], row[3], row[4]));
        } else { //train
            training_set.push_back(std::make_tuple(row[0], row[1], row[2], row[3], row[4]));
        }
    }

    printf("Captured %d gemm configurations...\n", inference_server_set.size());
    /// Insert CSV file reader here to populate inference_server_set and training_set
